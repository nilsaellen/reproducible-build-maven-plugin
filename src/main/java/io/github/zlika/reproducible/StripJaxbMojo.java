...
public final class StripJaxbMojo extends AbstractMojo
{
    ...
    /**
     * Text which allow to rely on the file generated by the JAXB xjc tool.
     * If provided, value is appended to the list of xjcGenerator configuration property values.
     */
    @Parameter(defaultValue = "", property = "reproducible.relyingCommentText")
    private String relyingCommentText;

    /**
     * The implementations of xjc JAXB generator for which files must be stripped.
     * Default value is COM_SUN_XML_BIND,ORG_GLASSFISH_JAXB.
     */
    @Parameter(defaultValue = "COM_SUN_XML_BIND,ORG_GLASSFISH_JAXB", property = "reproducible.xjcGenerators")
    private String xjcGenerators;
    ...
    private void fix() throws MojoExecutionException
    {
        ...
        final JaxbObjectFactoryFixer objectFactoryFixer = new JaxbObjectFactoryFixer(getRelyingCommentTexts(), charset);
    ...
    private boolean isJaxbFile(List<String> lines)
    {
        return lines.size() > JAXB_FILE_TIMESTAMP_LINE_NUMBER
                && lines.get(0).equals("//")
                && getRelyingCommentTexts().stream().anyMatch(lines.get(JAXB_FILE_JAXB_COMMENT_LINE_NUMBER)::contains)
    ...
    private List<String> getRelyingCommentTexts()
    {
        ArrayList<String> relyingCommentTexts = Arrays.stream(xjcGenerators.split(","))
                .map(String::trim)
                .map(XjcGenerator::valueOf)
                .map(XjcGenerator::getRelyingCommentText)
                .collect(Collectors.toCollection(ArrayList::new));
        if (StringUtils.isNotBlank(relyingCommentText))
        {
            relyingCommentTexts.add(relyingCommentText);
        }

        return relyingCommentTexts;
    }
    ...
    enum XjcGenerator
    {
        ...
        XjcGenerator(String relyingCommentText)
        {
            this.relyingCommentText = relyingCommentText;
        }

        public String getRelyingCommentText()
        {
            return relyingCommentText;
        }
    }
}
